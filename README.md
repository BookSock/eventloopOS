...
The Mac queue app shows task-session binding controls on review packets with `task_id`, auto-loads available sessions for the selected task, lets a human bind an existing Codex thread, then enables the recommended agent handoff from the same queue context.
