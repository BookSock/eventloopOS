...
class BrowserContextStoreOnlyRunnerTests(unittest.TestCase):
    ...
    def test_artifact_generation_captures_no_interrupt_flow(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "route_event",
                "assert_no_queue_item",
                "context_restore_plan",
                "context_restore_request",
                "context_restore_done",
            ],
        )
    ...
    def test_runner_self_test_contract(self) -> None:
        ...
        self.assertEqual(golden["expected_restore_request"]["status"], "pending")
...
class AmbientContextRouteRunnerTests(unittest.TestCase):
    def test_fixture_replay_passes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            result = self._run(Path(tmp))

        self.assertTrue(result.passed)
        self.assertEqual(result.scenario, AMBIENT_CONTEXT_ROUTE)
        self.assertEqual(result.mode, "fixture")
        self.assertEqual(result.details["event_id"], "evt_slack_ambient_context")
    ...
    def test_artifact_generation_captures_context_inferred_route(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact_dir = Path(tmp)
            self._run(artifact_dir)
        ...
        self.assertTrue(log["passed"])
        self.assertEqual([step["name"] for step in log["steps"]], ["attach_context", "inject_task_session"])
