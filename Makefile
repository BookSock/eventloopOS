.PHONY: test test-unit test-integration test-e2e test-e2e-browser test-e2e-native-browser test-e2e-native-browser-real-orchestrator test-e2e-mac-browser-restore test-e2e-macos test-e2e-macos-launch test-e2e-macos-ui test-e2e-macos-live-ui test-e2e-macos-live-handoff test-e2e-live test-e2e-live-boot test-e2e-live-full test-e2e-postgres-mcp-dogfood test-e2e-claude-real-followup test-e2e-provider-deeplink test-db-docker test-db-native dev-doctor dev-postgres dev-postgres-up dev-postgres-down dev-postgres-url dev-dogfood dogfood-check queue-add queue-lineage task-sessions task-bind task-messages mcp-sources mcp-preview mcp-route-once task-runtime-smoke live-aerospace voice-listen-command voice-stt-smoke run-queue-app test-contracts test-architecture test-mcp-stability fixtures-seed packets-golden proof-agent proof-live lint typecheck ci install
...
test-e2e-claude-real-followup:
	$(PNPM) test:e2e:claude-real-followup

test-e2e-provider-deeplink:
	$(PNPM) test:e2e:provider-deeplink
