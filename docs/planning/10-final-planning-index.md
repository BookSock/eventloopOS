...
- `app/orchestrator`: `GET /contexts?source=&task_id=&q=&limit=` lists and ranks stored context resources with event + route metadata, relevance score, and match reasons, giving agents a read path for passive browser captures and task-attached browser context. `browser_context_ranked_search` now gives harness-level proof that ranking beats pure recency.
