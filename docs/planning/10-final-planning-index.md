...
- `app/orchestrator`: `pnpm --filter @eventloopos/orchestrator run voice:send` forwards one transcript from env/stdin into `/voice/commands`; `voice:listen` consumes line-delimited local STT streams with optional wake phrase filtering, so local STT/wake-word tools can plug into the router without product-specific STT code.
...
- macOS menu bar summary/actions exist with unit-covered presentation text; next UI gap is stronger empty/error/loading states in the full window.
