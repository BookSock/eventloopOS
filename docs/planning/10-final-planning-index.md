...
- Task session control has fake, daemon-seeded dev controller, discovery/read API, terminal-backed adapter seams, Codex native thread controller seam, app-server method adapter, stdio process transport, and automatic task-hinted event injection; next gap is stronger task/thread association UX beyond title tags or env mapping.
