...
- `app/macos`: selected review packets with `task_id` now show task-session binding controls. The queue app can load `/task-sessions`, display matching bound sessions, bind an unbound session through `PUT /task-sessions/:id/task-binding`, then keep the recommended-action handoff in the same UI.
