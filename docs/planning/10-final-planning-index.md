...
- `app/orchestrator`: `GET /contexts?source=&task_id=&q=&limit=` lists and ranks stored context resources with event + route metadata, relevance score, and match reasons, giving agents a read path for passive browser captures and task-attached browser context. `POST /contexts/restore-plan` returns side-effect-free local instructions for browser-extension restore, URL open, or file open. `browser_context_ranked_search` proves ranking beats pure recency; `browser_context_store_only` proves passive capture plus context restore-plan.
