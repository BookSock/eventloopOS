...
- Opt-in macOS UI automation smoke passed locally with `EVENTLOOPOS_ENABLE_MACOS_UI_SMOKE=1 pnpm run test:e2e:macos-ui`; it opens the menu bar extra, opens the queue window, toggles Manual Mode, verifies the manual workspace capture banner/menu summary, and toggles back to Event Loop.
