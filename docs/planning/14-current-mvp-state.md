...
- `server.ts` is still large, but task-session injection policy, task followup audit, observability routes, task-session routes, context-restore routes, and queue routes have been extracted into smaller modules. It is down to roughly 1.1k lines after the queue route split. Continue splitting MCP/events/workspace routes before much more orchestrator feature width.

## Next Best Work

1. Continue extracting `server.ts` route/policy modules without behavior change, especially MCP sources, events, and workspace.
