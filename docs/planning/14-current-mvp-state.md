...
- `ORCHESTRATOR_CODEX_APP_SERVER_URL=ws://...` lets the orchestrator attach to an already-running websocket `codex app-server` instead of spawning a private stdio app-server. This supports the intended split where Ghostty shows the visible Codex cockpit while eventloopOS sends structured followups into the same thread through the control plane.
- `pnpm run dev:dogfood` now starts that shared websocket Codex app-server by default when `codex_app_server` task sessions are enabled, prints the `ws://` URL, and passes it to the orchestrator. Set `EVENTLOOPOS_DOGFOOD_SHARED_CODEX_APP_SERVER=0` to use the older private stdio path.
...
- Manual dogfood showed AeroSpace enable/disable is not a safe EventLoop mode toggle: even enabling then disabling AeroSpace can rearrange windows because AeroSpace imports/releases its workspace graph. EventLoop manual mode should stop EventLoop orchestration while leaving the workspace backend stable. For monitor hotkeys while AeroSpace is enabled, use exact window-id AeroSpace commands instead of native/Rectangle moves; native moves can snap back when AeroSpace still owns the old workspace assignment.
...
- `POST /agent-runs` is now a first-class normalized intake path for background agent runtimes. A Codex/Claude/fake/manual run in `waiting_approval` or `blocked` creates or updates exactly one review packet and one queue item. Moving the same run back to `running`, `completed`, `failed`, or `cancelled` clears the stale paper from the ready stack; blocking again reactivates the same paper instead of creating a duplicate. Omitted evidence/action arrays preserve prior run state so status-only updates do not erase audit context.
- `pnpm run agent:run -- --id <run-id> --provider codex --task "blog feedback" --status waiting_approval --summary "..."` gives local agents a simple CLI for the same `POST /agent-runs` path. This keeps runtime adapters and ad hoc subagents out of curl/JSON hand-writing when they need to return a paper to intake. `pnpm run test:e2e:agent-run-cli` starts a temp orchestrator and proves that CLI path creates a real queue item; `pnpm proof:live` includes it.
...
- `pnpm test:e2e:event-loop:codex-completion-workspace` passed locally on 2026-05-08. It starts one shared websocket Codex app-server, starts a real Codex task through the orchestrator, launches a sacrificial Ghostty TUI attached to the same native thread, proves `thread/loaded/list` sees that thread, proves an independent manual-style websocket client can add a turn to that same thread, routes Slack-like context into it afterward, detects a completed marker turn, queues human review through `POST /agent-runs`, approves it back into the same thread, then runs the isolated AeroSpace one-window move/restore proof and verifies no smoke windows/processes remain. Latest manifest: `artifacts/event-loop-codex-completion-workspace/2026-05-08T01-58-08-587Z-42510/manifest.json`, including `ghostty_cleanup.ok=true`.
