...
- Read-only `agent-slack` MCP wrapper exists for Jason dogfood. `config/mcp-sources.agent-slack.example.json` launches it over stdio, reads `EVENTLOOPOS_AGENT_SLACK_*` filters, shells out to `agent-slack search messages`, and maps compact Slack search output into `slack_message_to_event` items without exposing Slack write tools.
...
- `server.ts` is still large, but task-session injection policy, task followup audit, observability routes, and task-session routes have been extracted into smaller modules. Continue splitting route modules before much more orchestrator feature width.
