...
- `pnpm run agent:run -- --id <run-id> --provider codex --task "blog feedback" --status waiting_approval --summary "..."` gives local agents a simple CLI for the same `POST /agent-runs` path. This keeps runtime adapters and ad hoc subagents out of curl/JSON hand-writing when they need to return a paper to intake.
