...
| agent-slack MCP wrapper | `pnpm --filter @eventloopos/orchestrator test` | `agent_slack_events_server` parses noisy `agent-slack` JSON, maps compact Slack messages to Slack poll items, and exposes read-only `search_messages` over MCP stdio | Live Slack poll intentionally manual because it can reveal private messages. Use tight `EVENTLOOPOS_AGENT_SLACK_QUERY` filters. |
| Provider deeplink normalizers | `pnpm --filter @eventloopos/orchestrator test` | Slack/GitHub/Notion/Google Docs/Figma/browser URLs produce provider IDs, confidence reasons, and restore confidence in resource details | Per-provider live app scroll accuracy still needs dogfood metrics. |
| Local activity/metrics | `pnpm --filter @eventloopos/orchestrator test` | `/metrics` and `/activity` record event routing and queue done counters in-memory, and Postgres mode persists activity/counters | Task/session grouping is not implemented. |
| Dogfood review report | `pnpm --filter @eventloopos/orchestrator test` | `dogfood:review` formats text/JSON from `/metrics` and `/activity`, filters current-day activity, and fails cleanly when orchestrator is unavailable | Trend comparison by prior days is not implemented. |
