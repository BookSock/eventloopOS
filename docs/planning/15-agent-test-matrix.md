...
| Local activity/metrics | `pnpm --filter @eventloopos/orchestrator test` | `/metrics` and `/activity` record event routing and queue done counters in-memory, and Postgres mode persists activity/counters | Daily dogfood report still missing. |
