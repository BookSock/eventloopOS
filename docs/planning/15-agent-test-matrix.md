...
| Queue item lineage | `pnpm --filter @eventloopos/orchestrator test` | A selected queue item returns its review packet, related source events, queue-filtered activity, and sanitized task messages even after the item is done | `pnpm queue:lineage -- --queue-item-id <id>` against a running orchestrator. |
