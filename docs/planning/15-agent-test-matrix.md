...
| AgentRun CLI | `pnpm --filter @eventloopos/orchestrator test -- --test-name-pattern "agent run CLI"` | Local agents can upsert or fetch agent runs through `pnpm agent:run` / `pnpm agent:run:get` without hand-writing curl JSON | Use before telling external agents to call the AgentRun path directly. |
