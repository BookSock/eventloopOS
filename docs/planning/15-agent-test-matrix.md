...
| MCP source preview | `pnpm run mcp:sources`; `pnpm run mcp:preview <source-id>` | Agent can list sources and run non-routing, non-cursor-committing source preview before enabling route-once loop | Preview redacts title/body/summary text unless `EVENTLOOPOS_MCP_PREVIEW_INCLUDE_TEXT=1`; CLI also tolerates a pnpm `--` separator. |
...
| AgentRun intake papers | `pnpm --filter @eventloopos/orchestrator test`; `app/test-harness/bin/run-scenario agent_run_waiting_approval` | Waiting/blocked background agent runs become one durable review packet and one queue item; status-only updates preserve evidence/actions; resumed runs clear stale ready papers; re-blocked runs reactivate the same paper | Included in fixture E2E and `test:e2e:live`; use this before wiring a new runtime adapter into the queue. |
| AgentRun CLI | `pnpm --filter @eventloopos/orchestrator test -- --test-name-pattern "agent run CLI"`; `pnpm run test:e2e:agent-run-cli` | Local agents can upsert or fetch agent runs through `pnpm agent:run` / `pnpm agent:run:get` without hand-writing curl JSON; live smoke proves CLI -> real orchestrator -> queue item | Included in `pnpm proof:live`; use before telling external agents to call the AgentRun path directly. |
