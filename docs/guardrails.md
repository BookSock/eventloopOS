...
- [`ci.yml`](../.github/workflows/ci.yml) — runs `pnpm run ci` through [`bin/ci-linux`](../bin/ci-linux) on Ubuntu. This is the Linux-safe lane: contracts, lint, package typechecks, package tests, and non-macOS e2e smoke. It skips browser-download/live-mac work with `EVENTLOOPOS_SKIP_BROWSER_E2E=1`, ignores docs/artifacts-only pushes, cancels stale runs on newer pushes, uses read-only repo permissions, and has a 20-minute timeout.
