...
- [`ci.yml`](../.github/workflows/ci.yml) — runs `pnpm run ci` (proof-agent smoke + `proof:agent` umbrella). This covers the lint/typecheck/test/e2e proofs already encoded in `pnpm ci`.
- [`secret-scan.yml`](../.github/workflows/secret-scan.yml) — runs `gitleaks/gitleaks-action@v2` on every push to `main` and on every PR. Uses [`.gitleaks.toml`](../.gitleaks.toml) for allow-listing. History scan on initial install came back clean (no leaks across 250 commits).
