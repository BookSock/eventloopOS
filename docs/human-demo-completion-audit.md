...
| Screenshot capture for demo inspection | `bin/lab-mac-human-demo-setup` and `bin/human-demo-ready` default to `bin/lab-mac-desktop-capture` with `capture_mode=lab_desktop`, so routine checks do not focus the controller Mac's Screen Sharing window; `bin/local-screen-sharing-capture` remains available with `capture_mode=local_screen_sharing_window`, uses `capture_engine=coregraphics` by default, and only raises/focuses when `SCREEN_SHARING_RAISE=1` or `SCREEN_SHARING_CAPTURE_ENGINE=screencapture` is set; `bin/human-demo-ready` requires a valid latest proof screenshot plus a fresh readiness screenshot with readable image dimensions; `bin/human-demo-result-verify` rejects missing proof/readiness screenshots | Proven by guards; rerun setup for any older no-screenshot demo artifact |
