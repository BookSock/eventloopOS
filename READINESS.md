...
- Full reboot-level proof is integrated but currently failing: `artifacts/lab-runs/20260601-220403-reboot-proof/manifest.json` requested reboot and observed SSH go down, but SSH/Tailscale/VNC did not come back within 420s.
  ...
  - Current quick reachability evidence: `artifacts/lab-runs/20260602-011323-wait-online-quick/manifest.json` failed in one poll with `wait_mode=quick`, `connect_timeout_seconds=2`, `lab_tailscale_visible=true`, and `lab_tailscale_offline=true`, confirming the controller can see the lab node in Tailscale but it is offline without spending a 10-minute reboot wait.
  - Controller Tailscale status now reports `eventloop-lab-mac` (`100.71.111.25`) as offline, last seen about 2 hours before the latest probe.
...
- Postgres migration mismatch proof is implemented but opt-in because the lab currently needs a Postgres runtime such as Docker Desktop before `PRODUCT_READINESS_POSTGRES_MIGRATION_MISMATCH=1` can pass.
  - Current lab evidence: `artifacts/lab-runs/20260601-205226-lab-fault-postgres-migration-mismatch/manifest.json` reports `postgres_runtime_available=false` with fix text, while main dogfood health and screenshot capture stayed green.
- Fault-injection coverage now covers the named reliability matrix in release profile, except Postgres migration mismatch still needs a real lab Postgres runtime. Remaining release proof depth is fresh-clone, reboot, 3x flake confidence, signing/notarization, and Postgres migration mismatch with a real lab Postgres runtime.
