...
- human UX lab proof artifacts (`pnpm product:ux`) covering one-paper render
  screenshots, task workspace switching/memory, manual-mode Return + Restore /
  Return Here behavior, Codex idle auto-paper queueing with active task
  suppression, live send-back handoff, and final no-orphan cleanup,
